...
-- 2. Create the temporary helper procedure for composite & single indexes
DELIMITER $$

CREATE PROCEDURE SafeDropAndCreateJobHistoryIndex(
...
-- 3. Execute the procedure for single and composite indexes
CALL SafeDropAndCreateJobHistoryIndex('WorkstationJobHistory', 'idx_WorkstationJobHistory_Workcenter_ActiveFrom', '`WorkcenterId`, `ActiveFrom`');
CALL SafeDropAndCreateJobHistoryIndex('WorkstationJobHistory', 'idx_WorkstationJobHistory_SetupTechUserId', '`SetupTechUserId`');
CALL SafeDropAndCreateJobHistoryIndex('WorkstationJobHistory', 'idx_WorkstationJobHistory_WorkOrder_Sequence', '`WorkOrderId`, `SequenceNo`');

-- 4. Clean up the procedure from the database
DROP PROCEDURE IF EXISTS SafeDropAndCreateJobHistoryIndex;
